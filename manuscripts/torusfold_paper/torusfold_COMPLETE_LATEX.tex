...
\author{
Ziyi Yan\\
\textit{College of Computer Science and Technology, Jilin University, Changchun, China}\\
...
\begin{equation}
\text{PE}(i, 2h) = \sin\left(\frac{i}{10000^{2h/d}}\right)
\end{equation}
\begin{equation}
\text{PE}(i, 2h+1) = \cos\left(\frac{i}{10000^{2h/d}}\right)
\end{equation}
...
\begin{equation}
\text{TPE}(i, 2h) = \sin\left(\frac{2\pi \cdot h \cdot i}{L}\right)
\end{equation}
\begin{equation}
\text{TPE}(i, 2h+1) = \cos\left(\frac{2\pi \cdot h \cdot i}{L}\right)
\end{equation}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{fig1_tpe_periodicity.png}
...
\begin{enumerate}
\item \textbf{Equivariance vs. Flexibility:} Equivariant architectures (EGNN) guarantee SE(3) symmetry by construction, rotations and translations of the input graph produce corresponding transformations in predicted coordinates. This geometric inductive bias naturally constrains predictions to physically valid conformations, avoiding the coordinate explosion problem observed in Scheme 5. Flexible architectures (Transformer, Mamba) learn arbitrary mappings with higher capacity but risk producing invalid geometries.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{fig2_architecture_comparison.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{fig3_scheme6_architecture.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{fig5_tpe_ablation.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{fig6_data_quality.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{fig7_length_scaling.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{fig8_failure_analysis.png}
...
\item \textbf{Distance Constraints:} For selected pairs, constrain $d(p_i, p_j) \in [2.5, 4.0]$\AA
\end{enumerate}
...
\begin{itemize}
\item Coordinate explosion: Predicted coordinates diverged to $|x_i| > 100$\AA
\item RMSD: 245\AA\ (catastrophic)
