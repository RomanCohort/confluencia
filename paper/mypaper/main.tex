...
\begin{abstract}
\textbf{Summary:} No existing platform evaluates circRNA-specific therapeutic dimensions. We developed Confluencia for our own circRNA program, integrating six-compartment pharmacokinetic modeling (half-life matching literature within 4.1\%), uncertainty-adaptive five-dimension evaluation, and cross-modality Go/Conditional/No-Go recommendations. Immunogenicity scores are consistent with literature IFN data ($\rho{=}0.93$, $p{=}0.003$); 5D decisions remain stable under $\pm$50\% weight perturbation; gene signature target availability stratifies 3\,064 patients (log-rank $p{<}0.001$).\\
\textbf{Availability and implementation:} \url{https://github.com/RomanCohort/confluencia}, MIT license. Python 3.8+, no GPU required, GUI available.\\
