...
No circRNA crystal structures exist in PDB. Fewer than two dozen structural annotations appear in literature. TorusFold targets 3D atomic coordinate prediction, a task ViennaRNA cannot perform (ViennaRNA predicts secondary structure only). The transfer learning strategy (frozen backbone + trainable TPE/CircPairformer on ViennaRNA pseudo-labels) provides a practical path: secondary structure supervision teaches circular topology while the architecture prepares for 3D prediction. The diffusion-based structure head awaits circRNA 3D data. We propose circRNA-CASP as community validation.
...
TNBC subtype parameters derived from Jiang et al. \citep{jiang2019} Supplementary Table S2 (publicly available). Pharmacokinetic validation uses Wesselhoeft et al. \citep{wesselhoeft2018} published half-life data. Immunogenicity validation uses Chen et al. \citep{chen2019} published IFN-$\beta$ measurements. circRNA 3D structure data: not available (no public database exists).
