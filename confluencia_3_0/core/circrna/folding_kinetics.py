...
class KineticsFeatures:
    ...
    precision_tier: str                     # "viennarna_kinetics" or "coarse_estimate"
...
class FoldingKineticsPredictor:
    ...
    def predict(self, sequence: str) -> KineticsFeatures:
        ...
        if self._has_viennarna:
            ...
            if len(seq) > 500:
                warnings.warn(
                    "Fallback kinetics estimation for sequences >500nt may be "
                    "significantly inaccurate. Install ViennaRNA for reliable results."
                )
    ...
    def _estimate_suboptimal(self, sequence: str) -> List[SuboptimalStructure]:
        ...
        COARSE ESTIMATE: this heuristic has unknown accuracy relative to ViennaRNA
        barriers/treekin. Results should not be compared directly with
        ViennaRNA-derived metrics.
        """
        seq = sequence.upper()
        length = len(seq)
