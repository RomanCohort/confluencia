...
unmodified), enabling physiologically realistic 72 h trajectory
simulation; and (2) a sample-size-adaptive Mixture-of-Experts (MOE)
...
ADMET endpoints: hERG channel blockade, AMES mutagenicity, five CYP450
isoforms (1A2, 2C9, 2C19, 2D6, 3A4), BBB permeability, and
hepatotoxicity. Eighty-four SMARTS structural alerts include 25 PAINS
patterns [@baell2010], Brenk filters [@brenk2008], circRNA-specific
alerts (unmodified uridine motifs, long poly-A sequences), and general
toxicity patterns, organized into four severity categories.
Dose-dependent modeling estimates LD50, TD50, MTD, and therapeutic index
via Hill equation dose-response curves across 8 organ systems.
...
RNACTM half-life predictions match literature within 4.1% for unmodified
(6.24 h vs 6.0 h), $\Psi$ (15.61 h vs 15.0 h), and m6A (11.24 h vs
10.8 h) modifications; however, protein expression duration is
overestimated (97 h simulated vs 48 h reported, 102% error), indicating
that clearance dynamics require refinement. The MOE ensemble achieves
39.2% MAE reduction over Ridge ($p<0.001$, Cohen's $d=-6.36$) and 4.9%
over HGB---the strongest individual baseline. For drug efficacy
prediction where features are low-dimensional ($d=154$), Ridge
outperforms MOE ($R^2$ 0.984 vs 0.982), confirming that adaptive gating
adds unnecessary complexity when data is sufficient and feature space is
compact.

Feature ablation reveals that biochemical statistics (16 dimensions, 5%
of features) account for 65.5% of MAE increase upon removal, while
environment features are critical ($R^2=-0.016$ when removed). Removing
2048-bit Morgan fingerprints improves drug $R^2$ from 0.668 to
...
structural alerts include 25 PAINS SMARTS patterns, of which 9 unique
patterns are triggered by the test molecules with 100% detection rate.
