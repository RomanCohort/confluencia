...
def render_whitebox_panel(project_root: Path) -> None:
    with st.expander("白箱化解释（逻辑回归）", expanded=False):
        ...
        tree_max_depth = st.number_input("tree max_depth (0 表示不限制)", min_value=0, max_value=1024, value=0, step=1, key="wb_tree_max_depth")
        tree_min_leaf = st.number_input("tree min_samples_leaf", min_value=1, max_value=100, value=1, step=1, key="wb_tree_min_leaf")

        C_val = st.number_input("正则化强度（C, 值越小正则越强）", min_value=1e-6, max_value=1e6, value=float(st.session_state.get("wb_C", 1.0)), format="%g", key="wb_C")
        ...
        max_iter = st.number_input("最大迭代次数 (max_iter)", min_value=10, max_value=100000, value=int(st.session_state.get("wb_max_iter", 1000)), step=10, key="wb_max_iter")
        ...
        l1_ratio = st.number_input("l1_ratio (elasticnet 时有效)", min_value=0.0, max_value=1.0, value=float(st.session_state.get("wb_l1_ratio", 0.5)), step=0.05, key="wb_l1_ratio")
        tol = st.number_input("tol (优化停止阈值)", min_value=1e-12, max_value=1.0, value=float(st.session_state.get("wb_tol", 1e-4)), format="%g", key="wb_tol")
    ...
    with st.expander("跨模块解释器（拟合线性替代模型）", expanded=False):
        ...
        if df_mod is not None:
            ...
            sur_C = st.number_input("C (替代模型)", min_value=1e-6, max_value=1e6, value=1.0, format="%g", key="sur_C")
            sur_model_type = st.selectbox("替代模型类型", options=["logistic", "tree"], index=0, key="sur_model_type")
            sur_tree_max_depth = st.number_input("tree max_depth (0 表示不限制)", min_value=0, max_value=1024, value=0, step=1, key="sur_tree_max_depth")
            sur_tree_min_leaf = st.number_input("tree min_samples_leaf", min_value=1, max_value=100, value=1, step=1, key="sur_tree_min_leaf")
