...
from .predictor import DrugModelBundle, infer_env_cols, make_xy, train_bundle
from confluencia_shared.metrics import rmse as _rmse


@dataclass
class DrugTrainResult:
    model_path: str
...
@dataclass
class DrugEvalResult:
    model_path: str
    data_path: str
    metrics: Dict[str, float]


def train_drug_from_csv(
    *,
    data_path: str,
    model_out: str,
    ...
    resolved_env_cols = infer_env_cols(df, smiles_col=smiles_col, target_col=target_col, env_cols=env_cols)
...
def evaluate_drug_from_csv(
    *,
    ...
    bundle: DrugModelBundle = joblib.load(model_path)
    df = pd.read_csv(data_path)

    used_smiles_col = smiles_col or bundle.smiles_col
    ...
    metrics = {
        ...
        "invalid_smiles": float(invalid_smiles),
        "dropped_invalid_smiles": float(invalid_smiles if bool(drop_invalid_smiles) else 0),
    }

    return DrugEvalResult(
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Dedicated drug training and evaluation module")
    sub = parser.add_subparsers(dest="cmd", required=True)

    p_train = sub.add_parser("train", help="Train drug model from CSV")
    ...
    p_train.add_argument("--model", default="hgb", choices=["hgb", "gbr", "rf", "ridge", "mlp"])
    p_train.add_argument("--test-size", type=float, default=0.2)
    p_train.add_argument("--seed", type=int, default=42)
    p_train.add_argument("--featurizer-version", type=int, default=2)
    p_train.add_argument("--radius", type=int, default=2)
    p_train.add_argument("--n-bits", type=int, default=2048)
    p_train.add_argument("--keep-invalid-smiles", action="store_true")
    ...
    p_train.add_argument("--ridge-alpha", type=float, default=1.0)
    ...
    p_eval = sub.add_parser("eval", help="Evaluate trained drug model on CSV")
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if args.cmd == "eval":
        result = evaluate_drug_from_csv(
            model_path=args.model,
            data_path=args.data,
            smiles_col=(args.smiles_col or None),
