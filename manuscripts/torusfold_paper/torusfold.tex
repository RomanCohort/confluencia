...
Zero-training baseline using geometric constraint solving with simulated annealing to enforce bond lengths and approximate end-to-end proximity at the BSJ. Uses ViennaRNA circ-mode pair probabilities as distance constraints. On PDB circularized test set (N=30), achieves RMSD 25.47Å (median 23.35Å) with end-to-end distance 2.75Å---note this is approximate proximity, not covalent bond-forming closure (which requires sub-0.5Å tolerance after O3'–P bond conjugation). O(L) complexity. Without pair constraints (Scheme 2'), RMSD degrades to 85.39Å, demonstrating the importance of secondary structure priors.
...
\textbf{Limitations:} The test set contains only N=7 sequences (lengths 20-27 nt), which severely limits statistical power (SE(r)$\approx$0.18 on correlation claims; 95\% CIs span moderate to strong effects) and generalizability to longer or more diverse structures. Results should be interpreted as preliminary proof-of-concept rather than definitive performance estimates. A 20-27 nt RNA is a hairpin, not a biologically representative circRNA (typically 200-2000+ nt); scaling to full-length circRNAs is untested.
...
We intend to compare TorusFold against existing RNA structure prediction methods on our circularized test set (Figure~\ref{fig:baselines}). \textbf{These external baseline comparisons (IsRNA, AlphaFold3, FARFAR2) are not yet completed and are reported as pending in this version of the manuscript; the figure and the claim that Scheme 6 achieves competitive RMSD should therefore be read as anticipated rather than demonstrated.} The expected advantage is that Scheme 6 guarantees BSJ closure, a constraint that general-purpose RNA structure predictors like AlphaFold3 cannot enforce natively.
...
This $\sim$11\AA\ improvement coincides with higher data quality, but the two sets also differ in size (N$\approx$14,000 vs. N=7) and sequence length distribution, so the gain cannot be cleanly attributed to quality alone without matching the sets on length and source composition. With this caveat, the trend suggests that current methods have not reached their architectural ceiling---they may be limited by training data. We estimate that with 50-100 high-quality experimental circRNA structures, RMSD could potentially reach <10\AA.
...
We propose that the correct design for physics-guided attention in RNA structure prediction combines three elements: (1) FlashAttention for O(L) memory with dense GPU utilization, (2) ViennaRNA pair probabilities as soft attention bias rather than hard mask, and (3) learnable correction gates that allow the model to override the physics prior when warranted. Element (3) is partially realized by the iterative refinement procedure (Methods, Scheme 8), which uses the model's predicted geometry as evidence to recover ViennaRNA-missed pairs; a fully learned gate remains future work.

\subsection{Limitations}

We acknowledge several limitations:
\begin{enumerate}
\item \textbf{Small test set (N=7):} Limited statistical power precludes definitive conclusions; 95\% CIs on accuracy claims span moderate-to-strong effects.
...
\item \textbf{Limited evaluation metrics:} We report RMSD and BSJ closure error only; complementary structure-quality metrics (TM-score, GDT\_TS, lDDT) adopted in protein and RNA structure assessment are not yet computed and would strengthen local-quality and fold-level evaluation in a future revision.
\end{enumerate}
...
\textbf{Iterative Refinement (Geometric Feedback Loop):}
ViennaRNA's Top-$K$ selection misses pairs when the energy-based predictor mishandles pseudoknots or long-range contacts. To recover them, we use a multi-round refinement procedure. After the first diffusion round yields predicted coordinates $\mathbf{X}^{(1)} \in \mathbb{R}^{L \times 3}$, each subsequent round $r+1$ augments the candidate mask $\mathbf{M}$:

\begin{enumerate}
\item For each position $i$, examine only $j \in \mathcal{N}_r(i) = \{(i+k) \bmod L : |k| \leq r\}$, where $r=30$. This ring-surface neighborhood respects circRNA topology, since positions near the BSJ are neighbors despite large linear indices, at $O(L \cdot r)$ cost.
\item Retain only $(i,j)$ pairs whose bases are Watson-Crick or wobble complementary. This drops spurious spatial proximity caused by backbone stacking or prediction error.
\item Mark $(i,j)$ as recovered if $\lVert \mathbf{X}^{(r)}_i - \mathbf{X}^{(r)}_j \rVert_2 < d_{\text{thr}}$ ($d_{\text{thr}}=10$\,\AA) and $(i,j) \notin \mathbf{M}^{(r)}$.
\item Re-run diffusion with the enriched mask $\mathbf{M}^{(r+1)}$.
\end{enumerate}

Two to three recycle rounds are used. Beyond this, marginal gains diminish and self-predicted coordinates risk amplifying first-round errors. This procedure realizes the ``learnable correction gate'' discussed in Section~\ref{sec:sparse-limits}: instead of overriding the ViennaRNA prior through a learned gate alone, it uses the model's own geometric output as evidence to revise the prior.
