...
%% Complete LaTeX with correct figure references
...
% Graphics path - pointing to the correct figure directory
\graphicspath{{figures/}{D:/IGEM集成方案/confluencia_3_0/docs/paper/figures/}}
...
Module implementations include: (1) TNBC Simulacrum with spatial TME simulation (nine immune cell populations, six cytokines, three spatial compartments, subclonal evolution), (2) CirculaPK six-compartment pharmacokinetics capturing circRNA-specific bottlenecks (1-4\% endosomal escape), (3) circRNA-specific innate immune sensing via MDA5/dsRNA pathway with differential m6A suppression modeling, and (4) RL-ABM closed-loop sequence optimization. Preliminary benchmarks: immunogenicity scores correlate with Chen 2019 IFN-$\beta$ (Spearman r=0.91, N=7, 95\% CI [0.47, 0.99], preliminary—power 0.35 to distinguish from r=0.50); PK matches Wesselhoeft 2018 half-lives (4.1\% error, N=4, fitted parameters not independent validation); structure prediction backend achieves $\sim$2\AA\ RMSD with guaranteed BSJ closure via physics solver. Subtype comparison experiments show IM subtype responds 2.6x better than BLIS under identical chemotherapy (N=4 subtypes, 180 days; this difference reflects input parameterization rather than novel prediction—see Section~3.1). Three circRNA therapy mechanisms implemented (miRNA sponge, protein coding, immune stimulation) with event-driven treatment dispatch.
...
\textbf{Gap 2: circRNA innate immune sensing mechanisms are distinct from linear RNA.} circRNAs lack 5' termini, so RIG-I 5'-ppp sensing does not apply (Hornung et al., 2006); however, RIG-I can still sense short dsRNA blunt-end structures independently of 5'-ppp (Kohlway et al., 2013; Schlee and Hartmann, 2016), so the dominant circRNA immunogenicity pathway arises from longer dsRNA backbone structures sensed by MDA5 (Chen et al., 2019; Peisley and Hur, 2013) and modulated by intron identity. \textbf{Innovation 2}: We implement pathway-resolved immunogenicity scoring (MDA5/dsRNA, TLR7, TLR8, PKR) with differential m6A suppression modeling (90\%/30\%/20\% pathway-specific), correcting the oversimplified ``m6A reduces immunogenicity'' assumption and achieving statistically significant improvement over GC-only baseline ($\Delta$AIC = -8.2, p=0.004).
...
\textbf{Contribution Statement.} We present Confluencia 3.0 as a \textbf{computational platform and methods contribution} (not a therapeutic recommendation system) with four architectural innovations: (1) circRNA-specific PK model with literature-fitted parameters (not independently validated), (2) pathway-resolved immunogenicity scoring with differential m6A modeling, (3) subtype-adaptive TNBC simulation integrated with circRNA design, (4) extensible EventBus architecture enabling algorithm replacement without platform reimplementation. We additionally introduce three circRNA therapy mechanisms (miRNA sponge, protein coding, immune stimulation) with event-driven treatment dispatch. \textbf{All claims are hypothesis-generating pending wet-lab validation.} No specific dose, regimen, or modification recommendation in this manuscript should be interpreted as clinical guidance; circRNA-specific PK and immunogenicity parameters require experimental confirmation before therapeutic use.
...
\begin{enumerate}
\item \textbf{MDA5/dsRNA pathway}: Backbone dsRNA structures sensed by MDA5 (modeled as the dominant pathway for circRNA immunogenicity, though sequence-based prediction of MDA5 activation remains an open challenge—see Section~3.3)
...
\subsection{Module 4: Multi-Drug Combination with Mechanism-Driven Synergy}

\textbf{Motivation.} circRNA therapeutics in TNBC will likely be administered with checkpoint inhibitors or chemotherapy, yet no existing circRNA platform predicts combination effects. We integrate a multi-drug combination module that scores synergy from immune-cycle step complementarity rather than a hardcoded drug-pair lookup table.

\textbf{Single-drug PD parameters from Confluencia 2.0.} For each small molecule, the \texttt{DrugPredictionBridge} calls Confluencia 2.0's \texttt{predict\_full()} to obtain a binding/efficacy score together with PK/PD parameters (EC50, Emax, Hill coefficient) via \texttt{infer\_pkpd\_params} (3-compartment PK + sigmoid Emax PD link). The immune and inflammation inputs to the PK/PD inference are sourced from the TNBC Simulacrum TME state (TIL density, cytokine profile), closing the loop between tumor simulation and drug parameterization. The trained 2.0 Ridge MOE achieves $R^2 = 0.914$ on a held-out validation set ($N=330$; MAE $=0.033$).

\textbf{Mechanism-driven synergy scoring.} We implement four reference synergy models---Bliss independence, Loewe additivity, Highest Single Agent (HSA), and Chou-Talalay combination index (CI)---with the Bliss expected effect modified by a mechanism-driven boost:
\begin{equation}
E_{\text{observed}} = E_{\text{expected}}^{\text{Bliss}} \times (1 + \beta_{\text{mech}})
\end{equation}
where $\beta_{\text{mech}} \in [0, 0.25]$ is computed from immune-cycle step complementarity. Each drug target is mapped to one or more of six immune-cycle steps (1=antigen priming, 2=T cell priming, 3=trafficking, 4=recognition/killing, 5=direct tumor kill, 6=immune suppression). The complementarity is $1 - \mathrm{Jaccard}(\text{steps}_a, \text{steps}_b)$, with a canonical 0.25 boost for the vaccine (step 1) + checkpoint (step 4) pair reflecting antigen supply plus exhaustion relief, 0.20 for fully disjoint non-canonical pairs, and 0 for redundant same-step pairs. This replaces the previous hardcoded 20\% boost lookup table with an interpretable, mechanism-grounded model.

\textbf{Dose optimization.} A scipy-based optimizer balances expected efficacy, toxicity risk (overlapping toxicity profiles), and synergy score to identify Pareto-optimal dosing regimens across the dose-frequency grid.

\textbf{Validation.} The four synergy scorers are validated against the public NCI ALMANAC combination screening dataset (Holbeck et al., 2017; $>5000$ drug combinations $\times$ 60 cell lines), correlating predicted synergy scores with the gold-standard \texttt{COMBO\_SCORE}. Validation script: \texttt{experiments/validation\_nci\_almanac.py}. Results will be reported as Pearson $r$ with bootstrap 95\% CI and Bland-Altman statistics.
...
\textbf{Literature-constrained validation.} Six-compartment model validated against seven literature parameters with 100\% pass rate. Simulated half-lives match Wesselhoeft et al. (2018) experimental values: unmodified circRNA 6.24h vs 6.0h literature (4.1\% error), m6A-modified 11.24h vs 10.8h (4.1\% error), Psi-modified 15.61h vs 15.0h (4.1\% error); however, these half-life parameters were fitted to the Wesselhoeft 2018 data, so the 4.1\% error reflects parameter tuning rather than independent validation. \textbf{Major limitation—endosomal escape:} simulated 5.16\% vs literature 2\% (158\% error) on the rate-limiting step of the entire PK model. Because endosomal escape governs cytoplasmic concentration, protein expression, and therapeutic index, this 2.5-fold discrepancy propagates to all downstream predictions. The literature 2\% value derives from siRNA-LNP measurements in hepatocytes (Gilleron et al., 2013) and varies substantially by cell type, LNP formulation (ionizable lipid choice, PEG-lipid content), and cargo type (siRNA vs. mRNA vs. circRNA); the discrepancy likely reflects these confounders rather than model error alone. We recommend treating $k_{\text{escape}}$=0.025/h as an uncertain parameter and performing sensitivity analysis (Section~3.2.1) before using CirculaPK outputs for dose optimization. Tissue distribution matches Paunovska et al. (2018): liver 80\%, spleen 10\% (0\% error by design, but these values are also siRNA-derived and may not transfer to circRNA-LNP). Productive expression window: 40h vs 48h literature (16.7\% error).
...
\textit{Status:} Validated against literature values (4.1\% error on half-lives), but independent experimental confirmation needed. The endosomal escape parameter shows 158\% error, uncertainty in this rate-limiting step.
