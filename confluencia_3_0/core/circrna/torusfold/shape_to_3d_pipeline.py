...
def run_shape_to_3d_pipeline(
    ...
    # 4b. Process circBase sequences with simulated SHAPE
    if len(processed_samples) < total_needed:
        remaining = total_needed - len(processed_samples)
        print(f"\n  [4b] Processing circBase with simulated SHAPE ({remaining} remaining)...")

        for seq_idx, (seq_id, sequence) in enumerate(circbase_seqs):
            if len(processed_samples) >= total_needed:
                break

            full_id = f"circbase_{seq_id}"
            if full_id in processed_ids:
                continue

            # Predict unconstrained secondary structure first
            ss, mfe, pairs = predict_secondary_structure_with_shape(
                sequence,
                np.zeros(len(sequence)),  # Dummy reactivities
                use_shape=False,
            )

            # Simulate SHAPE reactivity from structure
            sim_reactivities = simulate_shape_reactivity(
                sequence,
                ss,
                noise_level=0.2,
                seed=seed + seq_idx,
            ...
            # Re-predict with simulated SHAPE (if ViennaRNA available)
            if HAS_VIENNA:
                ss_sim, mfe_sim, pairs_sim = predict_secondary_structure_with_shape(
                    sequence,
                    sim_reactivities,
                    use_shape=True,
                )
                ss, mfe, pairs = ss_sim, mfe_sim, pairs_sim
            ...
            if coords is not None:
                sample = ProcessedSample(
                    ...
                    secondary_structure=ss,
                    coords=coords,
                    reactivities=sim_reactivities,
                    source="shape_simulated",
                    mfe=mfe,
                    pair_constraints=pair_constraints,
                )
                processed_samples.append(sample)
    ...
    # 4c. Generate additional synthetic samples if needed
    if len(processed_samples) < total_needed:
        remaining = total_needed - len(processed_samples)
        print(f"\n  [4c] Generating synthetic samples ({remaining} remaining)...")

        bases = ['A', 'C', 'G', 'U']
        for i in range(remaining):
            L = rng.randint(min_len, max_len + 1)
            sequence = ''.join(rng.choice(bases, L))
            seq_id = f"synthetic_{len(processed_samples):05d}"
            ...
            sim_reactivities = simulate_shape_reactivity(
                ...
                ss,
                noise_level=0.2,
                seed=seed + len(processed_samples),
            )
