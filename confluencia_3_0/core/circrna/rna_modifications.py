...
class IRESSite:
    ...
    For circRNA, IRES is essential for cap-independent translation.
    Kozak context and ORF completeness are evaluated.
    """
    position_start: int
    position_end: int
    sequence: str
    ...
    orf_completeness: float = 0.0  # Fraction of ORF from IRES AUG to BSJ that is intact
    orf_length_nt: int = 0  # ORF length from IRES AUG to nearest stop codon or BSJ
...
class ModificationPredictor:
    ...
    def analyze(self, sequence: str, dot_bracket: Optional[str] = None) -> ModificationFeatures:
        ...
        # TODO: crosscheck_rmbase(m6a_sites) when database is available
    ...
    def _predict_m6a_sites(self, seq: str) -> List[ModificationSite]:
        ...
        SCOPE LIMITATION: This is motif-level screening, NOT site-level prediction.
        No cross-validation with m6A-Atlas or RMBase databases. Position-specific
        context (same site different tissues) is not modeled. For accurate site-level
        prediction, use dedicated tools like WHISTLE or m6A-Deep.
        """
        sites = []
    ...
    def _crosscheck_rmbase(self, seq: str, sites: List[ModificationSite]) -> List[ModificationSite]:
        """
        Placeholder for RMBase database cross-check.

        Future implementation will check predicted sites against known m6A sites
        in RMBase/m6A-Atlas. Currently returns sites unchanged.

        Args:
            seq: circRNA sequence
            sites: Predicted m6A sites

        Returns:
            Sites unchanged (placeholder behavior)
        """
        # TODO: Implement database cross-check
        # - Load RMBase/m6A-Atlas reference data
        # - Match predicted positions to known sites
        # - Annotate with experimental evidence
    ...
    def _compute_m6a_immunogenicity_effect(self, m6a_sites: List[ModificationSite]) -> float:
        """
        Compute m6A effect on immunogenicity — with directionality.

        m6A effect on immune response is CONTEXT-DEPENDENT, not always immune evasion:
        - Coding region m6A → YTHDF2-mediated degradation → reduced immunogenicity
          (Liu et al., 2022; Chen et al., 2019 — "immune evasion" pathway)
        - IRES/5'UTR-proximal m6A → YTHDF1-mediated translation enhancement →
          increased protein expression → potentially more immune visibility
          (Meyer et al., 2015; Wang et al., 2015)
        - m6A near dsRNA regions → destabilizes dsRNA stems → reduces RIG-I/PKR activation

        LIMITATION: Reader protein context (YTHDF1 vs YTHDF2 vs YTHDF3) and
        cell-type-specific effects are NOT modeled. The net direction is estimated
        from sequence context position alone — a coarse heuristic.
        """
        if not m6a_sites:
            return 1.0  # No m6A modification → no immunogenicity change

        evasion_weight = 0.0  # m6A sites that reduce immunogenicity
        enhancement_weight = 0.0  # m6A sites that may enhance immunogenicity

        for site in m6a_sites:
            prob = site.probability
            effect = site.effect

            if effect == "reduced_RIG_I_recognition":
                # m6A near GU-rich regions → RIG-I evasion (dominant effect)
                evasion_weight += prob * 0.40
            elif effect == "stability_modulation":
                # m6A near GC regions → dsRNA destabilization → PKR/RIG-I evasion
                evasion_weight += prob * 0.25
            elif effect == "translation_enhancement":
                # m6A enhancing translation → increased protein visibility →
                # potentially more immune activation (opposite direction)
                # This is a weaker, indirect effect
                enhancement_weight += prob * 0.15
            else:
                # Default: assume evasion (most common m6A effect on circRNA)
                evasion_weight += prob * 0.20

        # Net effect: evasion minus enhancement
        # Most m6A sites reduce immunogenicity, but some may increase it
        net_reduction = evasion_weight - enhancement_weight
        remaining = 1.0 - min(net_reduction, 0.30) + min(enhancement_weight, 0.15)

        # Clamp: m6A can reduce immunogenicity by up to 30%, but can also
        # increase it slightly (up to 15%) via translation enhancement
        return float(np.clip(remaining, 0.70, 1.15))
