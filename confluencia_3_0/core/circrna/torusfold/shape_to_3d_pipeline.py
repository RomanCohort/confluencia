...
def _simple_gradient_solve(seq_len: int, pair_constraints: list,
                           bond_length: float = 5.9) -> np.ndarray:
    """Generate 3D coords from constraints using gradient descent.

    Fallback when GeometricConstraintSolver fails.
    """
...
def run_shape_to_3d_pipeline(
    ...
    n_failed_solver = 0
    n_failed_ss = 0
    for idx, profile in enumerate(shape_profiles):
        ...
        # Fallback: gradient descent if solver fails
        if coords is None and pair_constraints:
            coords = _simple_gradient_solve(
                len(profile.sequence), pair_constraints, bond_length=5.9
            )
        elif coords is None:
            # Last resort: helical coords
            L = len(profile.sequence)
            radius = max(5.0, L * 2.8 / (2 * np.pi) * 0.8)
            coords = np.zeros((L, 3))
            for j in range(L):
                angle = np.deg2rad(32.7 * j)
    ...
    print(f"    Failed SS (no pairs): {n_failed_ss}")
