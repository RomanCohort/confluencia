%% main.tex — Confluencia Bioinformatics Application Note v3
%% 2026-05-30 | All metrics verified against benchmarks/results/*.json
%% v3: reframed as integration tool, not algorithm innovation
...
\title{Confluencia: An Integrated circRNA Drug Discovery Platform\\
with Six-Compartment Pharmacokinetic Modeling}
...
\begin{abstract}
\textbf{Summary:} Confluencia integrates circRNA-specific pharmacokinetic simulation, small-sample ensemble prediction, and ADMET evaluation into a single platform---addressing the fragmentation of existing specialized tools. RNACTM simulates circRNA delivery dynamics over 72\,h across six compartments using literature-derived parameters for five nucleotide modifications ($\Psi$, m6A, 5mC, ms2m6A, unmodified; half-life validated within 4.1\% for three modifications). A sample-size-adaptive Mixture-of-Experts ensemble achieves 39.2\% MAE reduction versus Ridge on epitope prediction ($N{=}300$), and ADMET covers 8 endpoint categories with 84 structural alerts (100\% PAINS recall).\\
\textbf{Availability and implementation:} \url{https://github.com/RomanCohort/confluencia}, MIT license. Python 3.8+, Linux/macOS/Windows. Docker and Confluencia Studio IDE available.\\
